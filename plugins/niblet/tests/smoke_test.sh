...
title "65. niblet-apply under Kimi runtime writes to .kimi/kb/"
...
[ -f "$KIMI_PROJECT/.kimi/kb/kimi-topic.md" ] && pass "KB written under .kimi/ when KIMI_SESSION set" \
  || fail "KB not found under .kimi/kb/"
[ ! -f "$KIMI_PROJECT/.claude/kb/kimi-topic.md" ] && pass "KB NOT written under .claude/" \
  || fail "KB incorrectly written to .claude/ under Kimi runtime"

title "66. niblet-status under Kimi runtime reports .kimi/ paths"
KIMI_STATUS_OUT="$(KIMI_SESSION=1 "$BIN/niblet-status" "$KIMI_PROJECT")"
echo "$KIMI_STATUS_OUT" | grep -q ".kimi" && pass "status mentions .kimi paths" \
  || fail "status missing .kimi paths under Kimi runtime"

title "67. niblet-apply UPDATE_MEMORY interruption template"
KIMI_MEM="$KIMI_PROJECT/.kimi/memory/feedback_interruptions.md"
jq -nc --arg f "feedback_interruptions.md" --arg c "- 2026-06-11: User stopped because wrong approach" \
  ...
  | KIMI_SESSION=1 "$BIN/niblet-apply" --project-root "$KIMI_PROJECT" >/dev/null
[ -f "$KIMI_MEM" ] && pass "interruption memory written under .kimi/memory/" \
...
title "68. niblet-apply-kimi forces .kimi/ without KIMI_SESSION"
# The wrapper must set NIBLET_RUNTIME=kimi internally; no ambient Kimi env.
unset KIMI_SESSION KIMI_HOME KIMI_WORK_DIR
FORCED_KB="$KIMI_PROJECT/.kimi/kb/forced-kimi.md"
...
[ -f "$FORCED_KB" ] && pass "Kimi wrapper wrote KB under .kimi/ without KIMI_SESSION" \
  || fail "Kimi wrapper did NOT write under .kimi/"
[ ! -f "$KIMI_PROJECT/.claude/kb/forced-kimi.md" ] && pass "Kimi wrapper did NOT write under .claude/" \
  || fail "Kimi wrapper incorrectly wrote under .claude/"
